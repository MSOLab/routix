...
### 2026-07-12 설계 3원칙 확정 (Protocol 계층 경계 · factory 주입 · contract 테스트가 계약, 계획서 §5.13 신설)

러너/orchestration 계층 **배선(wiring)**의 3원칙을 laadi 설계로 확정 — §5.9(기록)·
§5.3.1(정의)에 이어 계층을 **어떻게 잇는가**를 못박음. ① **계층 경계는 전부 Protocol**:
routix가 계승시킨 `Generic[ParametersT, SingleInstanceRunnerT]`/`ABC` 러너 체인
(`single_instance_runner.py:12`, `multi_instance_runner.py:12,24`,
`multi_scenario_runner.py:14,25-26`)을 concrete subclass에 대한 제네릭 대신 Protocol
seam으로 대체 — `MultiInstanceRunner`는 `run_single: Callable[[ParametersT],
SubroutineReportStatistics]`/좁은 `SingleRun` Protocol만, `MultiScenarioRunner`는
multi-runner 경계만 알고 single-runner의 존재를 아예 모름. pyright가 상속 없이 구조
검사하므로 기계 검증 유지 = **DIP(추상 의존)·ISP(좁은 인터페이스) 정합**(지금 구조는
"구체 서브클래스에 제네릭"이라 사실 DIP 위반). **TypeVar 유지/제거 기준** = "필드·반환
타입에 나타나느냐"가 아니라 **TypeVar가 무엇을 가리키느냐** — 클래스가 스스로 **생산해
도메인 API로 노출하는 대상**을 가리키면 유지(`SingleInstanceRunner`의
`SubroutineControllerT`: `get_controller() -> SubroutineControllerT` `:131`이 컨트롤러를
생산하고 `ctrlr` 필드 `:15`로 노출, 도메인 자식이 반환 타입에 의존 — **유지**),
**오케스트레이션하는 하위 계층**(ctor `type[T]`·`self.runners` 컬렉션 필드)을 가리키면
제거(multi/scenario의 러너 TypeVar: ctor `type[T]` 주입 + `self.runners: list[...]`
`multi_instance_runner.py:64`·`multi_scenario_runner.py:69`에 나타나도 하위 러너를 담기만
하는 통로라 Protocol이 대체 — **제거**).
**reach-through 트레이드오프 명시**: 상위 러너가 하위의 도메인 특화 필드에 타입 안전
접근 불가 → **반환 통계 객체를 넓히는 게 정답**(계층 관통은 애초에 계층 위반). ②
**factory/함수 주입**: `s_i_runner_class: type[SingleInstanceRunnerT]`/`m_i_runner_class`
식 `type[T]` 클래스 주입을 factory·callable 주입으로 대체 → "모든 하위 러너가 같은
생성자 시그니처" 강제가 사라져 계층이 **진짜 독립**. ③ **contract 테스트가 1차 계약**:
round-trip/conformance 테스트가 경계의 실행 가능한 정의 — Protocol seam마다 공용 pytest
스위트("이 Protocol을 구현하면 이 스위트를 통과"). 에이전트 중심 개발에서 형태 계약(타입)
보다 **행동 계약(테스트)이 더 강한 안전망**. §10-1 context-callable 판단("bound method는
Python 클래스 의미론을 경계에 새김")의 **러너 계층 상향 일반화**이며, 타입 힌트 자체의
가치는 오히려 상승(버리는 것은 "구체 서브클래스에 대한 제네릭 사슬"이지 정적 타이핑이
아님). 계획서 §5.13 신설 + §2.2(파이프라인 shape만 계승·wiring 교체)·§3.3(원칙5
contract 테스트가 계약 신설) 개정, §4(seam 전면화)·§5.3·§5.3.1(CSR 자식 컨트롤러 주해)·
§6.1(S5 시그니처 적합=Protocol 구조 적합)·§7(import 방향 테스트)·§10-1 연동.
...
그리고 이 override가 3계층 전부에서 일어나는 것(ffc_cmax) 자체가 routix의
`Generic`/`type[T]`/`ABC` 러너 체인이 강제하는 **병렬 상속 계층**의 실증 —
2026-07-12 §5.13(Protocol 경계·factory 주입)이 겨냥하는 통증. (routix 자체는 §0대로
그대로 유지; wiring 재구현은 laadi의 greenfield 몫이라 이 병행 수정 범위에는 없음.)
