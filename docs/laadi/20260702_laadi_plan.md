...
갱신: 2026-07-12 — 설계 3원칙 확정: Protocol 계층 경계 · factory 주입 · contract
테스트가 계약 (§5.13 신설; §2.2·§3.3(원칙5 신설) 개정, §4·§5.3·§5.3.1·§6.1·§7·§10-1 연동)
...
- **SRP bottom-up 러너 파이프라인**: controller → single-instance → multi-instance
  ...
  아티팩트**로 흐른다 (POST_PROCESS_ONLY와 외부 도구가 같은 면을 씀). 계승하는 것은
  이 **디스크 파이프라인 shape뿐** — routix의 `Generic[...]`/`type[T]`/`ABC` 러너
  wiring(concrete subclass에 대한 제네릭 + 클래스 주입)은 계승하지 않고 Protocol seam
  + factory 주입으로 재구현한다 (§5.13).
...
5. **contract 테스트가 계약의 본체다 (2026-07-12 확정).** 경계의 실행 가능한 정의는
   형태 계약(JSON Schema·타입 시그니처, §2.1 P1)이 아니라 그것을 통과시키는 **공용
   contract 테스트**다 — "이 Protocol/kind를 구현하면 이 공용 pytest 스위트를 통과해야
   한다". 에이전트 중심 개발에서는 형태 계약보다 **행동 계약(테스트)이 더 강한 안전망**
   이다(형태는 시그니처만, 행동은 소비자·생산자가 실제로 합의하는 것). round-trip
   테스트는 원칙 1의 동반물에서 경계의 **1차 정의로 승격** — Contract-First의 "계약에서
   테스트를 생성하고 계약을 부연하지 말라"의 laadi 판(§5.6·§5.13, Protocol seam마다 공용
   스위트).
...
**주해 (2026-07-12)**: 이 "같은 클래스의 자식 컨트롤러" 인스턴스화는 routix 시대 호스트
코드의 선례일 뿐 laadi의 목표 형태가 **아니다**. laadi에서 자식 시퀀스는 controller
subclass 인스턴스화가 아니라 프레임워크의 Protocol seam(`context.run_subflow` = 자식
frame subtree, §5.9·§5.13)으로 몰린다 — CSR 예제를 subclass 인스턴스화 처방으로 오독하지
말 것.
...
### 5.13 계층 경계는 Protocol, 확장은 factory 주입 (2026-07-12 확정)

§5.3/§10-1이 subalgorithm 경계에서 내린 **"callable Protocol > subclass"** 판단을
**러너 계층 전체로 상향 일반화**한다(§10-1의 근거 — "bound method는 Python 클래스
의미론을 경계에 새김" — 이 러너 사슬에 그대로 적용). routix가 계승시킨 러너 사슬은
concrete subclass에 대한 제네릭 + `type[T]` 클래스 주입 + `ABC`다:

- `single_instance_runner.py:12` — `Generic[ParametersT, SubroutineControllerT], ABC`
- `multi_instance_runner.py:12,24` — `Generic[ParametersT, SingleInstanceRunnerT], ABC`;
  ctor `s_i_runner_class: type[SingleInstanceRunnerT]`
- `multi_instance_concurrent_runner.py:13,24` — 동형(같은 제네릭 + `type[T]` 주입)
- `multi_scenario_runner.py:14,25-26` —
  `Generic[ParametersT, SingleInstanceRunnerT, MultiInstanceRunnerT], ABC`;
  ctor `m_i_runner_class`·`s_i_runner_class: type[...]`

이 사슬은 도메인 프로젝트(FFcDDW 등)가 **모든 계층을 서브클래싱**해야 하는 **병렬 상속
계층(parallel hierarchy)**을 강제한다. "구체 서브클래스에 대한 제네릭"이라 사실 DIP에
어긋나는 쪽 — laadi가 버리는 것이 바로 이 사슬이다.

**W1 — 계층 경계마다 `typing.Protocol`.** 각 러너는 concrete 하위 클래스에 대한
제네릭이 아니라 **좁은 Protocol seam**에만 의존한다 (wiring 원칙은 W-번호 —
§2.1의 계약 원칙 P1–P4와 구별):

- `MultiInstanceRunner`는 `SingleInstanceRunnerT` 대신
  `run_single: Callable[[ParametersT], SubroutineReportStatistics]` 또는 좁은
  `SingleRun` Protocol만 안다.
- `MultiScenarioRunner`는 **multi-runner 경계만** 알고 single-runner의 존재를 아예
  모른다.
- Protocol은 상속 없이 pyright가 **구조적으로** 검사하므로 기계 검증(§2.1 P4)은 그대로
  유지된다. 버리는 것은 "concrete subclass에 대한 제네릭 사슬"이지 정적 타이핑이
  **아니다** — 에이전트 중심 개발에서 타입 힌트의 가치(기계 검증 가능한 계약)는 오히려
  상승한다.
- **DIP·ISP 정합**: 좁은 인터페이스(ISP) + 추상에 의존(DIP)과 정확히 일치. §4의
  "(2a)/(2b) seam은 in-process Protocol"이 한 경계의 특례에서 **전 프레임워크 규칙**
  으로 승격한다.

**TypeVar 유지/제거 기준**: 갈림의 기준은 "필드·반환 타입에 나타나느냐"가 **아니라**
**TypeVar가 무엇을 가리키느냐**다 — 그 클래스가 **스스로 생산해 자기 도메인 API로
노출하는 대상**(자식이 반환 타이핑으로 의존하는 것)을 가리키면 **유지**, 그 클래스가
**오케스트레이션하는 하위 계층**(ctor `type[T]` 클래스 파라미터·`self.runners` 컬렉션
필드)을 가리키면 **제거**. `self.runners`처럼 하위 러너를 *담기만* 하는 필드는 소비처로
치지 않는다 — 생산물이 아니라 아래 계층을 위로 실어 나르는 통로이며, Protocol 리팩터가
통째로 대체하기 때문이다.

- **유지**: `SingleInstanceRunner`의 `SubroutineControllerT` — `get_controller() ->
  SubroutineControllerT`(`single_instance_runner.py:131`)가 컨트롤러를 **생산**하고
  `ctrlr: SubroutineControllerT` 필드(`:15`)로 노출한다. 도메인 자식이 이 반환 타입에
  의존하므로 **자신의 도메인 API가 실제 소비**. 이런 제네릭은 유지 가치가 있다.
- **제거**: `MultiInstanceRunner`의 `SingleInstanceRunnerT`, `MultiScenarioRunner`의
  `SingleInstanceRunnerT`·`MultiInstanceRunnerT` — 셋 다 ctor `type[T]` 클래스
  파라미터로 주입되고, 그중 둘은 `self.runners` 컬렉션 필드에도 나타난다
  (`MultiInstanceRunner.runners: list[SingleInstanceRunnerT]` `multi_instance_runner.py:64`,
  `MultiScenarioRunner.runners: list[MultiInstanceRunnerT]` `multi_scenario_runner.py:69`).
  하지만 이 필드는 *자신의 생산물*이 아니라 *오케스트레이션하는 하위 러너 컬렉션을
  담기만* 하는 것(=병렬 상속 강제)이라, 필드에 나타나도 제거 대상이다. Protocol
  리팩터가 `self.runners`를 `run_single: Callable`/좁은 `SingleRun`·multi-runner
  Protocol로 대체하면 이 컬렉션은 제네릭 TypeVar를 더는 지니지 않는다. 러너 경계는
  Protocol로 대체.

**W2 — factory/함수 주입.** `s_i_runner_class: type[SingleInstanceRunnerT]`/
`m_i_runner_class` 식 **`type[T]` 클래스 주입**을 **factory·callable 주입**
(`Callable[..., X]` 또는 context 제공 factory)으로 대체한다. `type[T]` 클래스 주입은
"모든 하위 러너가 **같은 생성자 시그니처**"라는 암묵 계약을 강제한다 — factory 주입은
이 시그니처 통일 강제를 없애 계층이 **진짜 독립**이 된다.

**reach-through 트레이드오프 (명시)**: Protocol 경계에서는 상위 러너가 하위 러너의
**도메인 특화 필드**에 타입 안전하게 접근할 수 없다. 필요하면 **반환 통계 객체
(`SubroutineReportStatistics`)를 넓히는 것이 정답**이며, 계층 관통(reach-through)
접근은 애초에 계층 위반이다.

**W3 — contract 테스트 연동 (원칙 승격은 §3.3 원칙 5)**: 각 Protocol seam의 **행동 계약은 공용 contract
테스트**로 규정한다 — "이 Protocol을 구현하면 이 공용 pytest 스위트를 통과해야 한다".
형태 계약(타입 시그니처)보다 **행동 계약(테스트)이 에이전트 중심 개발에서 더 강한
안전망**이다.

**연동**: §2.2(파이프라인 shape만 계승, wiring 교체) 개정, §3.3 원칙 5(contract
테스트가 계약) 신설, §4(seam 규칙 전면화)·§5.3·§5.3.1(CSR 자식 컨트롤러 주해)·§6.1
(S5 "시그니처 적합" = Protocol 구조 적합)·§7(import 방향 테스트)·§10-1(상향 일반화의
원본) 연동.
