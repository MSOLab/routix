...
| `subalgorithm_spec` | 정의·strict | input–mechanism–output 3요소 + `code_ref` + `emits_trajectory` |
...
- **frame 트리**: 실행 = frame의 트리. root frame = instance 실행 전체, subalgorithm
  호출 = 자식 frame, composite subalgorithm의 내부 단계(round, phase)도 frame — 재귀.
...
| S5 | subalgorithm | `docs/subalgorithms/<name>.md` + `specs/<name>.subalgorithm.json` + `code_ref` 구현 | spec 통과 + code_ref import 가능 + 시그니처 적합 |
...
- **RESUME 구현** — manifest·계약에 필요한 필드는 v1에 예약하되, 복원 메커니즘은
  v1.x (aladia처럼 "enum만 있는 상태"로 두지 않도록, v1에서는 RunMode에서 아예 빼는
  것도 검토).
...
3. run: frame 트리·방출 API → step 러너(불변식 강제) → single → multi(+concurrent,
   실패 계약) → scenario — **run_config 로더가 처음부터 러너를 구동** (aladia 실패의
   정면 교정)
