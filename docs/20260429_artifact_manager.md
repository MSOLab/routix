...
### 4.4 `SubroutineController` 보정 — log 격벽 + algorithm logger 명시 전달

#### 4.4.1 SC log 격벽의 rationale

상위 layer (main / MultiScenarioRunner / MultiInstanceRunner /
SingleInstanceRunner)는 **swap-on-root 방식의 현재 동작을 그대로 유지**한다
(Python 기본 동작에 맡김). 하지만 **SubroutineController + algorithm 사이만은
격벽**으로 분리한다. 이유:

- SC와 algorithm은 step 단위, batch 단위, dispatcher 단위로 record를 쏟아낸다.
  하나의 instance solve 동안 수천~수만 줄이 자연스럽게 발생.
- 그게 SIR.log로까지 propagate되면 SIR.log를 사람이 읽을 수 없게 된다 (SIR
  자신의 record는 instance 시작/종료/post-process 정도 수십 줄 수준이므로 SC
  소음에 묻힌다).
- 격벽은 **딱 한 군데(SIR ↔ SC)** 에만 둔다. 그 위는 swap-on-root로 충분히 분리
  되고, 그 아래(algorithm)는 의도적으로 SC와 같은 채널을 공유.

#### 4.4.2 구현

SC logger에 **직접** FileHandler를 부착하고 propagation을 끊는다:

```python
# SingleInstanceRunner가 controller 진입 직전에 호출 (idempotent helper로)
sc_logger = logging.getLogger(
    f"ffc_ddw_sum_et.orchestration.controller.{ins_name}"
)
sc_logger.propagate = False                       # 위로 새지 않게 차단
sc_logger.setLevel(logging.DEBUG)
fh = logging.FileHandler(layout.log_path("subroutine_controller", ...))
fh.setFormatter(_FILE_FMT)
sc_logger.addHandler(fh)

# console에도 SC record 보이게: root의 stream handler는 propagate=False라 못
# 받으므로 SC logger에도 별도 stream handler를 attach
sh = logging.StreamHandler()
sh.setLevel(_terminal_level(quiet, verbose))
sh.setFormatter(_TERMINAL_FMT)
sc_logger.addHandler(sh)
```

- 다른 모든 layer는 `propagate=True` (Python 기본) 유지. record가 root에 도달해
  현재 swap된 FileHandler로 흘러감.
- algorithm은 `__init__(..., logger: Logger | None = None)`으로 logger를 받고,
  ...
  적용.) algorithm은 `propagate=False`인 SC logger 인스턴스를 그대로 들고
  쓰므로, algorithm record도 SC.log 파일과 SC stream handler로만 흘러감.
...
| `<run_id>_SingleInstanceRunner.log` | SIR active 동안의 propagating record. **SC/algorithm record는 들어오지 않음** | swap-on-root + SC 격벽 |
| `<run_id>_SubroutineController.log` | SC + algorithm record | SC logger 직접 attach + `propagate=False` + 같은 logger 객체 공유 |

console에는 모든 record가 통합으로 보임 (root에 stream handler, SC logger에도
별도 stream handler).
...
- routix에 `routix.logging.LogScope`(swap-on-root용) +
  `routix.logging.IsolatedLogScope`(SC 격벽용) 두 컨텍스트 매니저를 도입 검토.
  후자는 named logger에 직접 handler를 부착하고 `propagate=False`를 세팅, 종료
  시 원복.
- 현재 `ffc_ddw_sum_et`의 `setup_logging` 안 `_MANAGED_TAG` 트릭은 그대로 들고
  오면 됨. 격벽 helper도 같은 tag 메커니즘으로 자기가 attach한 handler만 정리.
- 다만 routix는 logging 의존을 최소로 두는 방향이었으므로, 이 항목은 후순위
  (도입은 ffc 쪽에서 패턴이 안정된 다음 routix로 끌어올림).
...
- controller 진입 **직전**에 SC 격벽 helper 호출:
  `attach_isolated_log(sc_logger, layout.log_path("subroutine_controller", ...),
  quiet, verbose)`. 이 helper가
  - SC logger에 FileHandler 부착
  - `sc_logger.propagate = False`
  - SC logger에 별도 StreamHandler 부착 (console에서도 SC record 보이게)
  를 처리. controller 종료 후 `detach_isolated_log(sc_logger)`로 원복 (fd 누수
  방지). idempotent — child process가 worker init에서 다시 호출해도 안전.
...
- `self.logger`는 이미 `f"ffc_ddw_sum_et.orchestration.controller.{ins_name}"`
  named logger다 (controller_core.py:49). 이 logger가 SC 격벽의 주체.
  SingleInstanceRunner가 controller 진입 직전에 이 logger에 FileHandler +
  StreamHandler를 attach하고 `propagate=False` 세팅 (§ 4.4.2 / § 5.5).
...
- 결과적으로 `FFcDDWSubroutineController.run_*` (e.g. `run_neh_cp`, `run_mcf_lb`,
  ...
  넘기는 형태가 된다. algorithm은 `propagate=False`인 SC logger 객체를 그대로
  들고 쓰므로, algorithm record가 SC.log + SC stream handler로만 흘러가는 게
  자동 보장.
...
이번 SC 격벽은 logger 이름이 instance마다 다르므로(`controller.<ins_name>`)
...
```python
# routix.logging (또는 ffc_ddw_sum_et.logging_setup)

_MANAGED_TAG = "_routix_managed"

def attach_isolated_log(
    logger_name: str, file_path: Path,
    *, quiet: bool = False, verbose: int = 0,
    ...
    fh.setLevel(logging.INFO); fh.setFormatter(_FILE_FMT)
    setattr(fh, _MANAGED_TAG, True); log.addHandler(fh)
    sh = logging.StreamHandler()
    sh.setLevel(_terminal_level(quiet, verbose)); sh.setFormatter(_TERMINAL_FMT)
    setattr(sh, _MANAGED_TAG, True); log.addHandler(sh)
    return log

def detach_isolated_log(logger_name: str) -> None:
    log = logging.getLogger(logger_name)
    _sweep(log)                                     # (3) idempotent
    log.propagate = True
...
try:
    attach_isolated_log(sc_logger_name, layout.log_path("subroutine_controller", ...),
                         quiet=quiet, verbose=verbose)
    self.ctrlr = self.get_controller()
    self.ctrlr.run()
finally:
    detach_isolated_log(sc_logger_name)
```

`-q` / `-v`는 root stream handler와 SC stream handler 양쪽에 동일 level로 적용
(SC logger는 `propagate=False`라 root의 stream handler를 못 받기 때문에 별도
로 부착 필요).

**테스트로 검증할 시나리오**:

- `controller.run`이 정상 종료 → attached handlers가 detach로 사라지는가.
...
- [ ] `FFcDDWSingleInstanceRunner.run` SC 격벽 lifecycle (§ 7.2):
      - controller 진입 직전 `attach_isolated_log(sc_logger, ...)`
      - `try/finally`로 `detach_isolated_log(sc_logger)` 보장
      - attach helper 안에 defensive sweep (`_MANAGED_TAG` 청소)
      - SC logger에 file + stream handler 둘 다 attach (console 노출)
...
- [ ] tests:
      ...
      - `tests/test_sc_isolated_log.py` (§ 7.2 4-시나리오 검증: 정상 detach,
